...
MODEL_PATH = PROJECT_ROOT / "cnn_bilstm_stage2_grid_lr0.0001_bs8_mw3_lr0.0001_ep8_bs8.pt"
...
st.caption("Gunakan model Stage-2 manual untuk memproses audio retail secara lokal.")
...
def load_model_cached(model_path: Path, label_maps: LabelMaps) -> torch.nn.Module:
    model = load_model(model_path, label_maps, device=DEVICE)
...
def ensure_artifacts() -> Optional[str]:
    ...
    if not MODEL_PATH.exists():
        return f"Model Stage-2 tidak ditemukan di {MODEL_PATH}"
    return None
...
model = load_model_cached(MODEL_PATH, label_maps)
